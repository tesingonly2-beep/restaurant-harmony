...
const allOrders = [
  { id: "#A-2041", channel: "Dine-in", table: "T-12", customer: "Walk-in", items: 4, total: 3856, status: "Preparing", time: "5 min" },
  { id: "#A-2040", channel: "Dine-in", table: "T-04", customer: "Walk-in", items: 2, total: 1800, status: "Served", time: "12 min" },
  { id: "#A-2039", channel: "Takeaway", table: "—", customer: "M. Patel", items: 6, total: 5688, status: "Ready", time: "8 min" },
  { id: "#A-2038", channel: "Dine-in", table: "T-08", customer: "Walk-in", items: 3, total: 2720, status: "Preparing", time: "3 min" },
  { id: "#A-2037", channel: "Delivery", table: "—", customer: "S. Khan", items: 5, total: 4992, status: "Out for delivery", time: "20 min" },
  { id: "#A-2036", channel: "Online", table: "—", customer: "via Swiggy", items: 2, total: 2280, status: "Confirmed", time: "1 min" },
];
...
function OrdersPage() {
  ...
  const orders = allOrders.filter((o) => channel === "All" || o.channel === channel);
            ...
            <tbody>
              {orders.map((o) => (
                <tr key={o.id} className="border-b border-border/40 hover:bg-muted/40">
                  ...
                  <td className="py-2 px-3 font-medium">{formatINR(o.total)}</td>
                  ...
                  <td className="py-2 px-3 text-right">
                    <Button variant="ghost" size="sm" onClick={() => notify(`Opened ${o.id}`, `${o.channel} · ${formatINR(o.total)}`)}>
                      View
                    </Button>
                  </td>
                </tr>
